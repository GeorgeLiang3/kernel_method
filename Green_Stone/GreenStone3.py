...
import json 
from gempy.core.tensor.tensorflow_graph_uncon1 import TFGraph
import tensorflow as tf
...
class Model(object):
    def __init__(
        ...
        self.extent = [696000, 747000, 6863000, 6930000, -20000, 200]
        self.geo_data = gp.create_model('Greenstone')
        data_path = 'https://raw.githubusercontent.com/cgre-aachen/gempy_data/master/'

        # Importing the data from csv files and settign extent and resolution
        self.geo_data = gp.init_data(self.geo_data, self.extent, self.regular_grid_resolution,
        ...
        # self.geo_data.set_centered_grid(self.xy_ravel, resolution=[20, 20, 20], radius=[800,800,1500])
        
        # self.geo_data.set_centered_grid(self.xy_ravel, resolution=self.center_grid_resolution, radius=self.model_radius)
...
###########
    ...
    def get_graph_input(self):
        ...
        len_series_i = tf.constant(len_series_i,tf.int32)
        len_series_o = tf.constant(len_series_o,tf.int32)
        len_series_w = tf.constant(len_series_w,tf.int32)
        n_surfaces_per_series = tf.constant(n_surfaces_per_series,tf.int32)
        n_universal_eq_T = tf.constant(n_universal_eq_T,tf.int32)
        is_erosion = tf.constant(is_erosion,tf.bool)
        weights_vector = tf.constant(weights_vector,self.dtype)
        scalar_fields_matrix = tf.constant(scalar_fields_matrix,tf.bool)
        mask_matrix = tf.constant(mask_matrix,self.dtype)
        block_matrix = tf.constant(block_matrix,self.dtype)
    ...
    def compute_indices(self,number_of_points_per_surface):
        ref_positions = tf.cumsum(
                    tf.concat([[0], number_of_points_per_surface[:-1] + 1], axis=0))
        indices = [tf.squeeze(tf.where(tf.reduce_sum(tf.one_hot(ref_positions,tf.reduce_sum(number_of_points_per_surface+1),dtype = tf.int32),axis=0)==0)).numpy().tolist(),
                    tf.squeeze(tf.where(tf.reduce_sum(tf.one_hot(ref_positions,tf.reduce_sum(number_of_points_per_surface+1),dtype = tf.int32),axis=0)==1)).numpy().tolist()]
        return indices
    
    def create_tensorflow_graph(self, input, gradient = False):
        ## prepare some constant for the looping
        N_iteration = (np.diff(input[5])+1)
        
        indices = self.compute_indices(input[0])
        #need to fix
        npf_indices_list = tf.where(tf.reduce_sum(tf.one_hot([0], self.len_points, dtype='int32'), axis=0)==1)
        # get the iterating list out as a constant input to the graph, bacause it's given bug in Hessian
        iter_layer_per_surface = np.diff(input[5])+2
        self.TFG = TFGraph(input, self.fault_drift,
                ...
                self.C_o, self.rescale_factor,
                rest_ref_indices = indices,
                npf_indices_list = npf_indices_list,
                N_iteration = N_iteration,
                slope = 1000, dtype = self.tfdtype, gradient = gradient,iter_layer_per_surface = iter_layer_per_surface)
    
    # def calculate_grav(self,surface_coord, values_properties):
        
    
    def compute_model(self,gradient = False):
